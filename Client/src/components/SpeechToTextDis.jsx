...
const SpeechToTextDis = () => {
  ...
  // States for dynamic questions setup flow
  const [setupStep, setSetupStep] = useState('select_type'); // 'select_type', 'upload_file', 'choose_question', 'active_interview'
  const [practiceType, setPracticeType] = useState(null); // 'hr' or 'technical'
  const [techSource, setTechSource] = useState('resume'); // 'resume' or 'jd_text'
  ...
  const handleStartHR = async () => {
    try {
      ...
      setUploadError(null);
      const response = await makeAuthenticatedRequest(`${API_BASE_URL}/api/interview/questions/hr`);
      if (!response.ok) throw new Error("Failed to fetch HR question");
      const resData = await response.json();
      if (resData.success && resData.question) {
        setSpeechQuestion(resData.question);
        setSetupStep('active_interview');
      } else {
        throw new Error("Invalid response from server");
      }
    } catch (err) {
      console.error(err);
      setUploadError("Could not connect to server. Starting with local fallback question.");
      setSpeechQuestion({
        id: "hr_local_1",
        category: "Behavioral",
        difficulty: "Medium",
  ...
  const handleTechnicalSubmit = async (e) => {
    ...
    try {
      ...
      const token = localStorage.getItem('auth_token');
      const response = await fetch(`${API_BASE_URL}/api/interview/questions/technical`, {
        method: 'POST',
        body: formData,
        headers: {
      ...
      if (!response.ok) throw new Error("Failed to generate questions. Verify file type and try again.");
      const resData = await response.json();
      
      if (resData.success && resData.questions && resData.questions.length > 0) {
        setGeneratedQuestions(resData.questions);
        setSetupStep('choose_question');
      } else {
        throw new Error("No questions were generated by the server.");
      }
    } catch (err) {
      console.error(err);
      setUploadError(err.message || "Failed to communicate with the server.");
    } finally {
      setIsGenerating(false);
    }
  };
  ...
  const renderSetupUI = () => {
    if (isGenerating) {
      return (
        <div className="flex items-center justify-center min-h-screen w-full bg-gradient-to-br from-gray-50 to-gray-100 p-8">
          <div className="flex flex-col items-center max-w-md text-center p-8 bg-white rounded-2xl shadow-xl border border-gray-100 animate-pulse">
            ...
            <h3 className="text-xl font-bold text-gray-800 mb-2">Preparing Your Interview</h3>
            <p className="text-gray-600 text-sm">
              Our AI engine is processing your selection and generating custom-tailored interview questions. This may take a few seconds...
            </p>
          </div>
        </div>
      );
    ...
    if (setupStep === 'select_type') {
      return (
        <div className="flex flex-col items-center justify-center min-h-screen w-full bg-gradient-to-br from-gray-50 to-gray-100 p-8">
          <div className="max-w-4xl w-full text-center space-y-8 animate-fadeIn">
            <div>
              ...
              <p className="mt-3 max-w-2xl mx-auto text-xl text-gray-500 sm:mt-4">
                Select your focus area and let AI design custom interview scenarios for you.
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mt-12">
              ...
              <div 
                ...
                className="bg-white p-8 rounded-3xl shadow-md hover:shadow-xl transition-all duration-300 transform hover:-translate-y-2 border border-gray-100 cursor-pointer flex flex-col items-center text-center group"
              >
                <div className="w-16 h-16 bg-indigo-50 text-indigo-600 rounded-2xl flex items-center justify-center text-3xl mb-6 group-hover:bg-indigo-600 group-hover:text-white transition-all duration-300">
                  🎯
                </div>
                ...
                <p className="text-gray-500 leading-relaxed mb-6 text-sm">
                  Practice situational and behavioral questions designed to test leadership, collaboration, problem-solving, and adaptiveness. Structured around STAR method evaluations.
                </p>
                <span className="mt-auto px-6 py-2 bg-indigo-50 text-indigo-700 font-semibold rounded-xl text-sm group-hover:bg-indigo-600 group-hover:text-white transition-all">
                  Start Practice
                </span>
              </div>
              ...
              {/* Technical Card */}
              <div 
                onClick={() => setSetupStep('upload_file')}
                className="bg-white p-8 rounded-3xl shadow-md hover:shadow-xl transition-all duration-300 transform hover:-translate-y-2 border border-gray-100 cursor-pointer flex flex-col items-center text-center group"
              >
                <div className="w-16 h-16 bg-purple-50 text-purple-600 rounded-2xl flex items-center justify-center text-3xl mb-6 group-hover:bg-purple-600 group-hover:text-white transition-all duration-300">
                  💻
                </div>
                <h3 className="text-2xl font-bold text-gray-800 mb-3 group-hover:text-purple-600 transition-colors">
                  Technical Mock Round
                </h3>
                <p className="text-gray-500 leading-relaxed mb-6 text-sm">
                  Upload your Resume or paste a Job Description to generate targeted coding, algorithm, framework, or system architecture questions. Tailored to your specific profile.
                </p>
                <span className="mt-auto px-6 py-2 bg-purple-50 text-purple-700 font-semibold rounded-xl text-sm group-hover:bg-purple-600 group-hover:text-white transition-all">
                  Configure Round
                </span>
              </div>
            </div>
    ...
    if (setupStep === 'upload_file') {
      return (
        <div className="flex flex-col items-center justify-center min-h-screen w-full bg-gradient-to-br from-gray-50 to-gray-100 p-8">
          <div className="max-w-2xl w-full bg-white p-8 md:p-10 rounded-3xl shadow-xl border border-gray-100 space-y-8 relative">
            
            {/* Back Button */}
            <button 
              onClick={() => setSetupStep('select_type')}
              className="absolute top-6 left-6 flex items-center text-sm font-semibold text-gray-500 hover:text-gray-800 transition"
            >
              ← Back
            </button>

            <div className="text-center pt-4">
              <h2 className="text-3xl font-extrabold text-gray-900">Configure Technical Round</h2>
              <p className="text-gray-500 mt-2 text-sm">Upload your profile details to generate tailormade questions.</p>
            </div>

            {/* Selection tab */}
            <div className="flex border-b border-gray-200 bg-gray-50 p-1 rounded-2xl">
              <button
                type="button"
                onClick={() => { setTechSource('resume'); setFile(null); setUploadError(null); }}
                className={`flex-1 py-3 text-sm font-semibold rounded-xl transition ${techSource === 'resume' ? 'bg-white shadow-sm text-purple-600' : 'text-gray-500 hover:text-gray-800'}`}
              >
                📄 Upload Resume
              </button>
              <button
                type="button"
                onClick={() => { setTechSource('jd_text'); setFile(null); setUploadError(null); }}
                className={`flex-1 py-3 text-sm font-semibold rounded-xl transition ${techSource === 'jd_text' ? 'bg-white shadow-sm text-purple-600' : 'text-gray-500 hover:text-gray-800'}`}
              >
            ...
            <form onSubmit={handleTechnicalSubmit} className="space-y-6">
              {techSource === 'resume' ? (
                <div className="space-y-2">
                  <label className="block text-sm font-bold text-gray-700">Upload your Resume (PDF, TXT, Word)</label>
                  <div className="border-2 border-dashed border-gray-300 hover:border-purple-500 transition rounded-2xl p-8 text-center flex flex-col items-center justify-center bg-gray-50 cursor-pointer relative group">
                    <input
                      type="file"
                    ...
                    <span className="text-sm font-semibold text-gray-700 group-hover:text-purple-600 transition">
                      {file ? file.name : "Click or drag your resume file here"}
                    </span>
                    <span className="text-xs text-gray-400 mt-1">Supports PDF, TXT, or DOCX (Max 10MB)</span>
                  </div>
                </div>
              ) : (
                <div className="space-y-2">
                  <label className="block text-sm font-bold text-gray-700">Paste Job Description (JD)</label>
                  <textarea
                    rows="6"
                    value={jdText}
                    onChange={(e) => setJdText(e.target.value)}
                    className="w-full border border-gray-300 p-4 rounded-2xl focus:ring-2 focus:ring-purple-500 focus:outline-none text-sm text-gray-700 resize-none bg-gray-50"
                    placeholder="Paste the target job description details here to assess skills specifically required by the role..."
                  />
                </div>
              )}
              ...
              <button
                type="submit"
                className="w-full py-4 bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-700 hover:to-indigo-700 text-white rounded-2xl font-bold shadow-md hover:shadow-lg transition transform hover:-translate-y-0.5 active:translate-y-0"
              >
    ...
    if (setupStep === 'choose_question') {
      return (
        <div className="flex flex-col items-center justify-center min-h-screen w-full bg-gradient-to-br from-gray-50 to-gray-100 p-8 animate-fadeIn">
          <div className="max-w-4xl w-full text-center space-y-8">
            <div>
              <h2 className="text-3xl font-extrabold text-gray-900 tracking-tight">Select Your Question</h2>
              <p className="mt-2 text-lg text-gray-500">Pick one of the generated questions to begin your practice round.</p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-8">
              {generatedQuestions.map((q, idx) => (
                <div 
                  ...
                  className="bg-white p-6 rounded-2xl shadow-md hover:shadow-xl border border-gray-100 hover:border-purple-400 cursor-pointer flex flex-col justify-between text-left transition transform hover:-translate-y-1.5 duration-200"
                >
                  <div>
                    <span className="px-2 py-0.5 bg-purple-50 text-purple-700 text-xs font-semibold rounded-md">
                      {q.category || "Technical"}
                    </span>
                    <h4 className="font-bold text-gray-800 text-md mt-3 mb-2 leading-snug">
                      {q.title}
                    </h4>
                  ...
                  <div className="mt-6 flex items-center justify-between border-t border-gray-100 pt-4 text-xs font-medium text-purple-600">
                    <span>Difficulty: {q.difficulty || "Medium"}</span>
                    <span className="underline">Practice →</span>
                  </div>
                </div>
              ))}
            </div>
            ...
            <button 
              onClick={() => setSetupStep('upload_file')}
              className="mt-8 text-sm font-semibold text-purple-600 hover:text-purple-800 transition block mx-auto"
            >
              ← Back to Upload
            </button>
          </div>
  ...
  const interviewQuestion = speechQuestion || {
    id: 1,
    category: "Behavioral",
    difficulty: "Medium",
    title: "Tell me about a challenging project you worked on",
